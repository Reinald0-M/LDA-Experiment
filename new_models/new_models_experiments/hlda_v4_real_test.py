...
# 1. Real World Data Loading (Fashion MNIST)
# ==========================================
def get_fashion_mnist_hierarchy(max_samples=10000):
    print("Fetching Fashion-MNIST from OpenML...")
    X, y = fetch_openml('Fashion-MNIST', version=1, return_X_y=True, as_frame=False, cache=True)
    y = y.astype(int)
    
    # 1: Footwear, 2: Tops/Outer, 3: Legs/Body
    hierarchy_map = {
        1: [5, 7, 9],       
        2: [0, 2, 4, 6],    
        3: [1, 3]           
    ...
    print(f"Data Loaded: {X_norm.shape[0]} samples.")
    return X_norm, np.array(final_labels_class), np.array(final_labels_cluster)
...
def grad_sq_norm(W, diff):
    """Returns ||W.T @ diff||^2 and its gradient w.r.t W"""
    proj = W.T @ diff
    val = np.dot(proj, proj)
    # Grad of x^T W W^T x w.r.t W is 2 * x * (W^T x)^T
    grad = 2 * np.outer(diff, proj)
    return val, grad

# ==========================================
# 3. Optimization Logic (Gaussian Stable)
# ==========================================
def joint_gradient_ascent_stable(S_B, S_WS, S_BS, subclass_means, parent_means, mu_tot, 
                             reg=1e-4, num_iters=10000, fix_hyperparams=None, verbose=True):
    dims = S_B.shape[0]
    np.random.seed(42) 
    W = np.random.randn(dims, 2); W, _ = np.linalg.qr(W)
    ...
    # --- STABILITY SETTINGS ---
    step_W = 1e-4     # Can be safely higher now that we use Gaussian
    step_hyp = 1e-4    
    gamma_rep = 0.5   # Width of Gaussian repulsion (1/sigma^2)
    ...
    for it in range(num_iters):
        ...
        # --- TERM 1: Gaussian Repulsion (Sibling Separation) ---
        # Objective: Minimize Sum( exp(-gamma * ||proj||^2) )
        # This pushes ||proj|| to be large.
        J_rep = 0.0; grad_J_rep = np.zeros_like(W)
        ...
        for c, sub_means in subclass_means.items():
            ...
            for i in range(num_sub):
                for j in range(i+1, num_sub):
                    diff = sub_means[i] - sub_means[j]
                    sq_dist, g_sq_dist = grad_sq_norm(W, diff)
                    
                    # Gaussian Kernel
                    # E = exp(-gamma * dist^2)
                    # dE/dW = E * (-gamma) * d(dist^2)/dW
                    E = np.exp(-gamma_rep * sq_dist)
                    J_rep += E
                    grad_J_rep += -gamma_rep * E * g_sq_dist
                    
        # --- TERM 2: Topology Hinge Loss ---
        # Objective: Minimize max(0, ||Anc||^2 - tau * ||Glo||^2)
        J_top = 0.0; grad_J_top = np.zeros_like(W)
        for c, sub_means in subclass_means.items():
            for mu_cs in sub_means:
                diff_anc = mu_cs - parent_means[c]
                sq_anc, g_sq_anc = grad_sq_norm(W, diff_anc)
                
                diff_glo = mu_cs - mu_tot
                sq_glo, g_sq_glo = grad_sq_norm(W, diff_glo)
                
                loss = sq_anc - tau * sq_glo
                if loss > 0:
                    J_top += loss
                    grad_J_top += (g_sq_anc - tau * g_sq_glo)

        # --- TOTAL RATIO ---
        # Maximize N / D
        # N = Trace(Between)
        # D = Trace(Mix) + l1 * Topology + l2 * Repulsion
        # (We minimize D, which minimizes Hinge and minimizes Gaussian -> maximizes separation)
        
        tr_num = np.trace(W.T @ S_B @ W)
        tr_den = np.trace(W.T @ S_mix @ W)
        
        N = tr_num 
        D = tr_den + l1 * J_top + l2 * J_rep + 1e-6
        ratio = N / D
        
        # Gradients
        grad_N_W = 2 * S_B @ W 
        grad_D_W = 2 * S_mix @ W + l1 * grad_J_top + l2 * grad_J_rep
        
        # Quotient Rule: (D * gN - N * gD) / D^2
        grad_J_W = (D * grad_N_W - N * grad_D_W) / (D**2)
        
        # Clip Gradient Norm for extra safety (prevent massive jumps at start)
        gnorm = np.linalg.norm(grad_J_W)
        if gnorm > 5.0:
            grad_J_W = grad_J_W * (5.0 / gnorm)
        ...
        # Update Hyperparams
        if not fix_hyperparams:
            # Simple gradient descent on D (since we want to minimize D part of ratio)
            # Actually, maximizing Ratio N/D w.r.t l1 means minimizing D... 
            # so l1, l2 naturally want to go to 0 to maximize objective.
            # We must PENALIZE l1, l2 being small, or fix them.
            # For this script, we assume Grid Search fixes them.
            pass
        ...
        if verbose and it % 100 == 0:
            print(f"Iter {it}: Ratio={ratio:.4f}, RepulsionCost={J_rep:.4f}", end='\r')
...
# ==========================================
def run_grid_search(S_B, S_WS, S_BS, sub_means, par_means, mu_tot, reg):
    print("Running Grid Search (Stable)...")
    ...
    for bundle in itertools.product(*values):
        params = dict(zip(keys, bundle))
        W_curr, _, _, _, _, hist, _ = joint_gradient_ascent_stable(
            S_B, S_WS, S_BS, sub_means, par_means, mu_tot, 
            reg=reg, num_iters=50, fix_hyperparams=params, verbose=False 
        )
        
        if hist[-1] > best_obj:
            best_obj = hist[-1]; best_params = params; best_W = W_curr.copy()
        
        count+=1
        if count % 10 == 0: print(f"Grid {count}/{total} | Best: {best_obj:.2f}", end='\r')
            
    print(f"\nDone. Best: {best_obj:.4f}")
    return best_params, best_W, best_obj

def viz(W, data, y, best_obj, hist_g):
    proj = data @ W
    plt.figure(figsize=(10,5))
    plt.subplot(1,2,1)
    plt.scatter(proj[:,0], proj[:,1], c=y, cmap='tab10', alpha=0.5, s=10)
    plt.title(f"Stable Projection (Obj={best_obj:.2f})")
    plt.subplot(1,2,2)
    plt.plot(hist_g)
    plt.title("Gradient Norm (Should be stable)")
    plt.yscale('log')
    plt.tight_layout()
    plt.savefig('stable_hlda.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    # 
    # PCA is critical here to handle non-Gaussian image data
    print("Applying PCA...")
    pca = PCA(n_components=100)
    data = pca.fit_transform(data_raw)
    
    # 2. Matrices
    Sb, Sw, Sbs, pm, sm, mt = compute_matrices(data, y_cls, y_clst, 100)
    
    # 3. Grid
    bp, bW, bObj = run_grid_search(Sb, Sw, Sbs, sm, pm, mt, 1e-4)
    
    # 4. Final Run
    print("Final Optimization...")
    W, _, _, _, _, h, hW = joint_gradient_ascent_stable(
        Sb, Sw, Sbs, sm, pm, mt, reg=1e-4, num_iters=5000, fix_hyperparams=bp
    )
    
    viz(W, data, y_cls, h[-1], hW)