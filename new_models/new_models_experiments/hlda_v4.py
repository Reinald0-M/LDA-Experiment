...
#     # Defaults
#     # NOTE: tau is now FIXED. It will not change during iterations.
...
#         # S_R (Numerator: Sibling Separation)
...
#         # Note: We NO LONGER calculate grad_tau because tau is fixed
#         for c, sub_means in subclass_means.items():
...
#         # Penalties: Centering Alpha + Decay L1/L2
#         pen_alpha = gamma_alpha * (alpha - 0.5)**2
#         pen_l = decay_l * (l1**2 + l2**2)
#         obj_total = ratio - pen_alpha - pen_l
...
# ==========================================
# 3. Optimization Logic (Stable + Verbose Control)
# ==========================================
def joint_gradient_ascent_v4(S_B, S_WS, S_BS, subclass_means, parent_means, mu_tot, 
...
def run_grid_search(S_B, S_WS, S_BS, sub_means, par_means, mu_tot, reg):
    ...
    for bundle in itertools.product(*values):
        ...
        # --- SILENCE THE INNER LOOP HERE (verbose=False) ---
        ...
        count += 1
        # Added extra whitespace padding to clear previous shorter lines
        if count % 10 == 0:
...
if __name__ == "__main__":
    ...
    # dim_list = [3,3,3,10,10,10,100,100,100,1000,1000,1000]
    # dim_list = [100,1000]
    for dims in dim_list:
        ...
        W, a, l1, l2, t, h, hW = joint_gradient_ascent_v4(
                ...
                verbose=True  # Turn printing back on for the final run
            )
