...
best_beta = None
best_W = None
grid_alpha = np.linspace(0, 1, 11)
grid_beta = np.linspace(0, 1, 11)
...
for alpha in grid_alpha:
    for beta in grid_beta:
        ...
        top_indices = np.argsort(eigvals)[::-1][:2]
        W = eigvecs[:, top_indices]
        obj_val = np.trace(W.T @ S_num @ W) / np.trace(W.T @ S_den @ W)
        if obj_val > best_obj:
            best_obj = obj_val
            best_alpha = alpha
            best_beta = beta
...
# 4. Projection and Plotting
data_proj = data_points @ best_W
import matplotlib.pyplot as plt
fig, axs = plt.subplots(2, 2, figsize=(12, 10))
fig.tight_layout(pad=4.0)
num_classes = len(unique_classes)
discrete_cmap_all = ListedColormap(plt.colormaps['viridis'](np.linspace(0, 1, num_classes)))

ax = axs[0, 0]
scatter_all = ax.scatter(data_proj[:, 0], data_proj[:, 1], c=labels_class, cmap=discrete_cmap_all, alpha=0.7)
ax.set_title("All Classes")
cbar_all = fig.colorbar(scatter_all, ax=ax, ticks=unique_classes)
cbar_all.set_label("Class")

positions = [(0, 1), (1, 0), (1, 1)]
for pos, c in zip(positions, unique_classes):
    ax = axs[pos]
    idx = np.where(labels_class == c)[0]
    X_c_proj = data_proj[idx, :]
    subclass_vals = np.array([lbl[1] for lbl in labels_cluster[idx]])
    unique_subs = np.unique(subclass_vals)
    discrete_cmap_sub = ListedColormap(plt.colormaps['plasma'](np.linspace(0, 1, len(unique_subs))))

    scatter_sub = ax.scatter(X_c_proj[:, 0], X_c_proj[:, 1], c=subclass_vals, cmap=discrete_cmap_sub, alpha=0.7)
    ax.set_title(f"Class {c}")
    cbar_sub = fig.colorbar(scatter_sub, ax=ax, ticks=unique_subs)
    cbar_sub.set_label("Subclass")

plt.suptitle(f"Hierarchical LDA with α={best_alpha:.2f}, β={best_beta:.2f}", fontsize=20)
plt.tight_layout()
plt.show()
