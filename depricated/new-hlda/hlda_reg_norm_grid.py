...
grid_lambda1 = [0, 0.1, 0.5, 1]
grid_lambda2 = [0, 0.1, 0.5, 1]
...
for lam1 in grid_lambda1:
    for lam2 in grid_lambda2:
            W0 = W_init.copy()
            W_opt = optimize_W_hlda(S_B, S_W, subclass_means, parent_means, W0, lam1, lam2, num_iters=200, step_size=1e-4)
            obj_val = hierarchical_lda_objective(W_opt, S_B, S_W, subclass_means, parent_means, lam1, lam2)
            print(f"Grid search (lambda1={lam1}, lambda2={lam2}): Final Objective = {obj_val:.4f}\n")
            if obj_val > best_obj:
                best_obj = obj_val
                best_l1 = lam1
                best_l2 = lam2
